...
export default function Hero() {
  ...
  return (
    // 'min-h-[100svh]' no mobile garante que ocupe a tela toda se precisar, 
    // mas 'h-auto' permite crescer mais se o texto for grande.
    // 'bg-[#1a3a52]' fica apenas como segurança, a imagem vai cobrir ele.
    <section className="relative w-full h-auto min-h-[600px] md:min-h-[80vh] flex flex-col justify-center overflow-hidden bg-[#1a3a52]">
      
      {/* CAMADA DE FUNDO (IMAGEM + OVERLAYS) */}
      <div className="absolute inset-0 w-full h-full z-0">
        <img
          ...
          // object-cover garante o "zoom" para preencher tudo sem deformar
          // object-top garante que o nome da faculdade não seja cortado
        ...
        {/* Overlay Escuro para dar leitura no texto (Mantido) */}
        <div className="absolute inset-0 w-full h-full bg-[#1a3a52]/60 md:bg-[#1a3a52]/50" />

        {/* Gradiente extra para o mobile (ajuda no contraste do título) */}
        <div className="absolute inset-0 w-full h-full bg-gradient-to-b from-black/70 via-transparent to-black/70 opacity-80 md:opacity-0" />

        {/* A TRANSIÇÃO SUAVE (FADE) */}
        {/* Fica colada no fundo (bottom-0), tem altura fixa curta (h-24) e vai do branco (background) para transparente */}
        {/* Isso conecta a imagem ao resto do site sem criar névoa no meio da foto */}
        <div className="absolute bottom-0 left-0 right-0 w-full h-24 bg-gradient-to-t from-background to-transparent z-10" />
      </div>

      {/* CONTEÚDO (TEXTO + BOTÃO) */}
      {/* py-20 garante que o conteúdo não encoste nas bordas superior/inferior */}
      <div className="container relative z-20 flex flex-col items-center text-center text-white space-y-8 px-4 py-24 md:py-0">
        ...
        <div className="animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-600 pt-4 pb-2">
          <GlowButton
